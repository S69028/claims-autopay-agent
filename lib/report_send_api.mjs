...
const RUNTIME_PRIVATE_DIR = process.env.VERCEL ? path.join('/tmp', 'claims-autopay-agent', 'private') : PRIVATE_DIR;
...
const REPORT_FILE_INDEX_RUNTIME_PATH = path.join(RUNTIME_PRIVATE_DIR, 'report_file_index.json');
const DELIVERY_LOG_PATH = path.join(PRIVATE_DIR, 'report_delivery_log.jsonl');
const DELIVERY_LOG_RUNTIME_PATH = path.join(RUNTIME_PRIVATE_DIR, 'report_delivery_log.jsonl');
...
function normalizeSupabaseBaseUrl(rawValue) {
  ...
  if (value.endsWith('/rest/v1')) {
    return value.slice(0, -'/rest/v1'.length);
  }
  return value;
}

function getSupabaseBaseUrl() {
  return normalizeSupabaseBaseUrl(env('SUPABASE_URL', 'https://ejgruldmrfbtalplwowr.supabase.co'));
}
...
function supabaseRestUrl(pathname) {
  ...
  return `${getSupabaseBaseUrl()}/rest/v1${cleanPath}`;
}

function supabaseWriteHeaders() {
  const key = getSupabaseWriteKey();
  if (!key) {
    throw new Error('SUPABASE_SERVICE_ROLE_KEY is required to write report_archive_fact');
  }
  return {
    apikey: key,
...
async function writeReportFileIndex() {
  const targetPath = process.env.VERCEL ? REPORT_FILE_INDEX_RUNTIME_PATH : REPORT_FILE_INDEX_PATH;
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.writeFile(targetPath, `${JSON.stringify(await buildReportFileIndex(), null, 2)}\n`, 'utf8');
}

async function upsertArchiveRow(row) {
  const response = await fetch(supabaseRestUrl('/report_archive_fact?on_conflict=snapshot_month'), {
    method: 'POST',
    headers: {
      ...supabaseWriteHeaders(),
  ...
  if (!response.ok) {
    throw new Error(`Supabase archive upsert failed (${response.status}): ${await response.text()}`);
  }
  const payload = await response.json();
  const nextRow = Array.isArray(payload) ? payload[0] : payload;
  if (!nextRow) {
    throw new Error('Supabase archive upsert returned no row');
  }
  return nextRow;
}
...
async function recordDeliveryLog({ reportArchiveId, recipients, sentAt, success, errorMessage = null }) {
  ...
  const targetPath = process.env.VERCEL ? DELIVERY_LOG_RUNTIME_PATH : DELIVERY_LOG_PATH;
  let existing = 0;
  try {
...
async function buildArchiveState({ reportMonth, reportPath, recipientCount, sentAt, success, notes }) {
  ...
  return {
    ...
    file_path: `../reports/${path.basename(reportPath)}`,
    checksum,
  ...
  try {
    ...
    try {
      ...
      writeJson(res, 200, {
        ...
        archive_table: 'report_archive_fact',
